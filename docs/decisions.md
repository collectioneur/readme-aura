...
- **Context**: Profile/project starter `readme.source.md` needed a single hero block matching the liquid/glow aesthetic, a centered `powered by readme-aura` footer, and safe fallbacks when API data is missing. Aura JSX used `github`/`repo` identifiers that were only injected when data existed, so optional chaining could throw `ReferenceError`. The shared `aura-hero-assets.ts` module was later removed: the full hero (keyframes + SVG + layout) is duplicated verbatim in each template file to match the canonical reference block.
- **Decision**: (1) Inline glow SVG + keyframe CSS in both `source-profile.ts` and `source-project.ts` (no shared module). (2) Always pass `github: github ?? null` and `repo: repoData ?? null` in the build JSX context so `github?.` / `repo?.` work in every block. (3) Interpolate `owner`/`repo` into generated source for static fallbacks (e.g. avatar URL, display name). (4) Hero uses the flat flex layout from the reference (`alignItems: 'center'`, SVG `top: 0`, text column `zIndex: 10`).
- **Alternatives**: Shared `aura-hero-assets.ts` (removed — duplicate markup preferred for a single canonical copy-paste shape). Require users to guard with `typeof github !== 'undefined'` (rejected — too noisy).
